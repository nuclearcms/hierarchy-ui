<template>
  <div :class="isLoaded ? 'reveal is-loaded' : 'reveal'">
    ...
    <div class="paper">
      <ContentsHeader
        :permission="$can('write_contents')"
        :resource="resource"
        :editingLocale="null"
        :errors="form.errors"
        :canHaveMoreTranslations="canHaveMoreTranslations"
        :canDeleteCurrentTranslation="false"
        :contentId="contentId"
      />
      ...
      <form
        method="POST"
        action="/api/contents"
        @submit.prevent="requestUpdate('contents', '/settings')"
        ...
        <div class="paper__body paper__body--noside">
          <div class="paper__main">
            <FormBody
              :schema="schema"
              v-model="form"
              :readonly="!$can('write_contents') || resource.is_locked == 1"
            />
          </div>
        </div>
        <SubmitFooter
          v-if="$can('write_contents') && resource.is_locked == 0"
          :config="{ icon: 'save' }"
          v-model="form"
        >
          <div class="control" v-if="contentStatus < 50">
            <button
              class="button icon-only is-warning"
              type="submit"
              @click.prevent="saveAndPublish"
              :disabled="form.errors.anyErrors"
            >
...
export default {
  ...
  data() {
    return {
      ...
      breadcrumbs: [
        {
          to: { name: "contents.index" },
          text: this.$root.trans.get("hierarchy::contents.multiple"),
        },
      ],
      ...
      tabs: [
        {
          route: "contents.edit",
          label: "hierarchy::contents.single",
          active: false,
        },
        {
          route: "contents.settings",
          label: "hierarchy::contents.settings",
          active: true,
        },
        {
          route: "contents.statistics",
          label: "hierarchy::contents.statistics",
          active: false,
        },
      ],
      form: new Form({
      ...
      schema: [
          ...
          },
          {
            ...
            options: {
              ...
              choices: [
                {
                  value: 30,
                  label: this.$root.trans.get("hierarchy::contents.status_30"),
                },
                {
                  value: 40,
                  label: this.$root.trans.get("hierarchy::contents.status_40"),
                },
                {
                  value: 50,
                  label: this.$root.trans.get("hierarchy::contents.status_50"),
                },
                {
                  value: 60,
                  label: this.$root.trans.get("hierarchy::contents.status_60"),
                },
              ],
            },
          },
        ...
        [
          ...
          },
          {
            ...
            label: this.$root.trans.get(
              "validation.attributes.children_display_mode"
            ),
            options: {
              required: true,
              choices: [
                {
                  value: "list",
                  label: this.$root.trans.get("hierarchy::contents.mode_list"),
                },
                {
                  value: "tree",
                  label: this.$root.trans.get("hierarchy::contents.mode_tree"),
                },
              ],
            },
          },
  ...
  mounted() {
    ...
    Event.$on("resource-loaded", function (data) {
      if (
        (data.content_type.hides_children || data.hides_children) &&
        self.tabs.length == 3
      )
        self.tabs.unshift({
          route: "contents.children",
          label: "hierarchy::contents.children",
          active: false,
        });
    });

    Event.$on("content-tree-modified", function (data) {
      if (data.id != self.resource.id) return;
