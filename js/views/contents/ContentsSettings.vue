...
export default {
	...
	mounted() {
		...
		Event.$on('resource-loaded', function(data) {
			if((data.content_type.hides_children || data.hides_children) && self.tabs.length == 3) self.tabs.unshift({ route: 'contents.children', label: 'hierarchy::contents.children', active: false})
		})
	},
	beforeDestroy() {
		Event.$off('resource-loaded')
